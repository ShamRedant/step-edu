...
// GET - Fetch all homework files for a lesson
export async function GET(req, { params }) {
  try {
    const { id } = await params;

    const query = `
      SELECT hf.*, a.email as uploaded_by_email
      FROM homework_files hf
      LEFT JOIN admins a ON hf.uploaded_by = a.id
      WHERE hf.lesson_id = $1
    ...
    return NextResponse.json(
      { success: false, message: "Failed to fetch homework files" },
      { status: 500 }
    );
  }
}

// POST - Upload a homework file
export async function POST(req, { params }) {
  try {
    ...
    const dueDate = formData.get("due_date"); // Optional due date
    ...
    // Save file
    const fileData = await saveFile(file, "homework-files", lessonId);

    // Save to database
    const query = `
      INSERT INTO homework_files (
        lesson_id, file_name, original_name, file_path, 
        file_type, file_size, mime_type, uploaded_by, due_date
      )
